...
class UserSyllabusSeeder extends Seeder
{
    /**
    ...
    {
        DB::table('user_syllabus')->truncate();
        ...
        foreach ($users as $user) {
            // Mengambil data dari table syllabus_histories
            $syllabusHistories = DB::connection('be_brainy')
                ->table('syllabus_histories')
                ->where('user_id', $user->id)
                ->get();

            // Grouping data dari table syllabus_histories berdasarkan nama
            $groupedHistories = $syllabusHistories->groupBy('name');
